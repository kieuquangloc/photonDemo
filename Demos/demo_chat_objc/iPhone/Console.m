...
- (instancetype) initWithFrame:(CGRect)frame
{
	if(self=[super initWithFrame:frame])
	{
		...
		lineWidth = (self.frame.size.width*0.95f)/(self.font.pointSize*0.6f); // width to height ratio for Courier New is 0.6
		ary = [[EGMutableArray alloc] initWithObjects:@"", nil];
		initialized = true;
	}
	return self;
}
