...
, selector(selector)
{
	...
	bg->drawPolygon(pp, 4, ROOM_LIST_BG_COLOR, 0, ROOM_LIST_BORDER_COLOR);
...
bool ListController::removeItem(const JString& name)
{
	unsigned int i = 0;
...
void ListController::removeAllItems()
{
...
void ListController::scrollTo(float nextY)
{
	...
	for(unsigned int i=0; i<menu->getChildrenCount(); ++i)
	{
		...
		if(centerOffs < FADE_START)
			...
			op = (unsigned char)(255 * (1 - centerOffs) / (1 - FADE_START));
		else
			op = 0;
		item->setOpacity(op);
