...
public:
	...
	{
		OutputListener::debugReturn(debugLevel, str);
	}
...
- (void) createGameDirectAllToAll
{
	viewMasterServer.hidden = true;
	gApp->setLastInput(INPUT_3);
}

- (void) createGameDirectMasterToAll
{
	viewMasterServer.hidden = true;
	gApp->setLastInput(INPUT_4);
}
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
	...
	unsigned short buttonWidth = 240, buttonHeight = 40;
	
	UIButton* button0 = [UIButton buttonWithType:UIButtonTypeRoundedRect];
	button0.frame = CGRectMake(viewMasterServer.frame.size.width/2-buttonWidth/2, viewMasterServer.frame.size.height*1/5-buttonHeight/2, buttonWidth, buttonHeight);
	[button0 setTitle:@"Create Game" forState:UIControlStateNormal];
	button0.titleLabel.textColor = [UIColor whiteColor];
	...
	button01.frame = CGRectMake(viewMasterServer.frame.size.width/2-buttonWidth/2, viewMasterServer.frame.size.height*2/5-buttonHeight/2, buttonWidth, buttonHeight);
	[button01 setTitle:@"Create Game Direct All To All" forState:UIControlStateNormal];
	button01.titleLabel.textColor = [UIColor whiteColor];
	button01.backgroundColor = [UIColor blackColor];
	[button01 addTarget:self action:@selector(createGameDirectAllToAll) forControlEvents:UIControlEventTouchUpInside];
	[viewMasterServer addSubview:button01];
	
	UIButton* button02 = [UIButton buttonWithType:UIButtonTypeRoundedRect];
	button02.frame = CGRectMake(viewMasterServer.frame.size.width/2-buttonWidth/2, viewMasterServer.frame.size.height*3/5-buttonHeight/2, buttonWidth, buttonHeight);
	[button02 setTitle:@"Create Game Direct Master To All" forState:UIControlStateNormal];
	button02.titleLabel.textColor = [UIColor whiteColor];
	button02.backgroundColor = [UIColor blackColor];
	[button02 addTarget:self action:@selector(createGameDirectMasterToAll) forControlEvents:UIControlEventTouchUpInside];
	[viewMasterServer addSubview:button02];
	
	UIButton* button1 = [UIButton buttonWithType:UIButtonTypeRoundedRect];
	button1.frame = CGRectMake(viewMasterServer.frame.size.width/2-buttonWidth/2, viewMasterServer.frame.size.height*4/5-buttonHeight/2, buttonWidth, buttonHeight);
